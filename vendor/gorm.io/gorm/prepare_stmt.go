...
func (db *PreparedStmtDB) prepare(ctx context.Context, conn ConnPool, isFood bool, query string) (Stmt, error) {
	db.Mux.RLock()
	if stmt, ok := db.Stmts[query]; ok && (!stmt.Food || isFood) {
		db.Mux.RUnlock()
	...
	cacheStmt := Stmt{Food: isFood, prepared: make(chan struct{})}
